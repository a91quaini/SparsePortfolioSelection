test_that("Full selection with identity sigma and ones mu works", {
  ...
  result <- compute_mve_sr(mu = mu, sigma = sigma, selection = 0:2, do_checks = TRUE)
...
test_that("Empty selection defaults to full selection", {
  ...
  mu <- c(1, 2, 3)
  sigma <- diag(3)
  expected <- compute_mve_sr(mu = mu, sigma = sigma, selection = 0:2, do_checks = TRUE)
...
test_that("Subset selection works correctly", {
  ...
  result <- compute_mve_sr(mu = mu, sigma = sigma, selection = c(0, 2), do_checks = TRUE)
  expect_equal(result, expected, tolerance = 1e-6)
})
...
test_that("do_checks errors on non-square sigma", {
  ...
  expect_error(
    compute_mve_sr(mu = mu, sigma = sigma, selection = 0:1, do_checks = TRUE),
...
test_that("do_checks errors on out-of-bound selection", {
  ...
  expect_error(
    compute_mve_sr(mu = mu, sigma = sigma, selection = c(0, 3), do_checks = TRUE),
